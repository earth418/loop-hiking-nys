...
function dfs(starting_pt) {
    let starting_ptindex = starting_pt.ftIndex;
    let starting_lineindex = starting_pt.parentPathIndex;

    let complete_paths = [];
    let queue_pt = {"prevPts" : [], "point" : starting_pt};

    // let pt_cursor = ptindex;
    let queue = [point];
    while (queue.length > 1) {
        let pt = queue.shift();
        if (pt.point.ftIndex == starting_ptindex && pt.point.parentPathIndex == starting_lineindex) {
            complete_paths.push(pt.prevPts);
        }

        if (!explored_pts.has(pt_to_set(pt.point))) {
            explored_pts.add(pt_to_set(pt));
            // [].concat()
            let new_prevpts = pt.prevPts.concat([pt.point]);
